...
function theme_loop_columns() {
	return apply_filters( 'theme_loop_columns', 3 ); // 3 products per row
}
...
function theme_woocommerce_scripts() {
	...
	if ( is_rtl() ) {
		wp_enqueue_style( 'theme-woocommerce-style', get_template_directory_uri() . '/core/woocommerce/css/woocommerce-rtl.css', $theme_version );
	} else {
		wp_enqueue_style( 'theme-woocommerce-style', get_template_directory_uri() . '/core/woocommerce/css/woocommerce.css', $theme_version );
	}
}
...
function theme_related_products_args( $args ) {
	$args = apply_filters( 'theme_related_products_args', array(
		'posts_per_page' => 3,
		'columns'        => 3,
	) );
...
function theme_thumbnail_columns() {
	return intval( apply_filters( 'theme_product_thumbnail_columns', 4 ) );
}
...
function theme_products_per_page() {
	return intval( apply_filters( 'theme_products_per_page', 12 ) );
}
