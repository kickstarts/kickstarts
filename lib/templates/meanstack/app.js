...
var config            = require('./config'),                        // Get configuration
...
db.on('disconnected', function () {
    ...
    console.log('✗ Disconnected from %s'.red.bold, config.db.host);
});

db.on('open', function() {
    winston.info('MongoDB connected!');
    console.log('✓ Connected to %s'.green.bold, config.db.host);
});
...
process.on('SIGINT', function() {
    db.close(function () {
        ...
        console.log('✗ Disconnected from %s. App has shutdown.'.red.bold, config.db.host);
        process.exit(0);
    });
});
...
server.listen(app.get('port'), function() {
    ...
    // Exit on CTRL+C
    process.on('SIGINT', function () {
        winston.info(config.locals.name + ' has shudown.');
        console.log(
            '\n✓ ' + config.locals.name + ' has ' + 'shutdown'.green.bold,
            '\n✓ ' + config.locals.name + ' was running for ' + Math.round(process.uptime()).toString().green.bold + ' seconds.'
        );
        process.exit(0);
    });
});
