...
gulp.task('browserify', function() {
    gulp.src(config.task.bundle.src)
        .pipe(__.browserify( {
            // shim: config.task.bundle.shims,
            insertGlobals : true,
            debug : true
        }))
...
gulp.task('minify', function() {
    gulp.src(config.task.minify)
        ...
        .pipe(gulp.dest(config.dist.script + '/bundle.min.js'));
});
...
gulp.task('images', function() {
    gulp.src(config.task.imagemin)                   // Read images
        .pipe(__.changed(config.dist.images))        // Only process new/changed
        .pipe(__.imagemin({
            optimizationLevel: 5,
            progressive: true,
            interlaced: true
